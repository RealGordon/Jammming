...
export const Spotify = {
  getUserName: function () {
    ...
    if (headers) delete headers["Content-Type"];else return Promise.reject("error no headers");
  ...
  savePlaylist: function (name, uris) {
    ...
    return this.getUserName().then(() => this.getPlaylistID(name)).then(id => postPlaylist(id));
  },
  search: function (term) {
    ...
    return fetch(endpoint, {
      ...
      if (Array.isArray(items)) return items;else return [];
    });
  },
  ...
  getAccessToken: function () {
    ...
    if (match) {
      ...
      headers = {
        Authorization: "Bearer " + (accessToken = match[1])
      };
